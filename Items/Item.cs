...
namespace RoomGenerator
{
    ...
    public class Item
    {
        ...
        protected Color m_BaseColour = Color.White;
        protected Color m_ActualColour = Color.White;

        //each item will have different texture representing it on screen
        protected Texture2D m_Texture;
        public bool isAlive = true;

        public Item(Vector2 a_Position, Texture2D a_Texture, ItemType a_Type)
        {
            m_Texture = a_Texture;
            m_Position = a_Position;
            m_StartPos = m_Position;
            m_Dimensions = new Vector2(25);
            m_Type = a_Type;
        }
        ...
        public void Draw()
        {
            if (isAlive)
            {
                Utility.Instance.SB.Draw(m_Texture, GetRect, m_ActualColour);
#if DEBUG
                Utility.Instance.SB.DrawString(Utility.Instance.LabelsFont, m_Type.ToString(), m_Position, Color.White);
                Utility.Instance.SB.DrawString(Utility.Instance.LabelsFont, m_Type.ToString(), new Vector2(m_Position.X - 1, m_Position.Y), Color.Black);
#endif
            }
        }
        ...
        public bool Intersects(Rectangle a_Rect)
        {
            Rectangle collision = new Rectangle(GetRect.X + (int)(m_Dimensions.X * 0.5f),
                                                ...
                                                GetRect.Width,
                                                GetRect.Height);
            return collision.Intersects(a_Rect);
        }
