            ...
            for (int i = 0; i < wallLength; ++i)
            {
                switch (a_Direction)
                {
                    case Direction.NORTH:
                        AddWall(new Wall(m_Position + new Vector2(0, -i * m_WallDimensions.Y), m_WallDimensions, Color.DeepPink));
                        break;
                    case Direction.SOUTH:
                        AddWall(new Wall(m_Position + new Vector2(0, i * m_WallDimensions.Y), m_WallDimensions, Color.DeepPink));
                        break;
                    case Direction.EAST:
                        AddWall(new Wall(m_Position + new Vector2(i * m_WallDimensions.X, 0), m_WallDimensions, Color.DeepPink));
                        break;
                    case Direction.WEST:
                        AddWall(new Wall(m_Position + new Vector2(-i * m_WallDimensions.X, 0), m_WallDimensions, Color.DeepPink));
                        break;
